...
bool OrochiLayerSelector::select(int layer_num)
{
    ...
    LOG_INFO("Selecting layer: %d", layer_num);

    const std::string layer_target = "O_LAYER_" + std::to_string(layer_num);
    if (find_layer_directly(layer_num)) {
        LOG_INFO("Layer %d found directly", layer_num);
        return click_layer(layer_target);
    }

    LOG_INFO("Layer not found directly, trying to swipe...");
    if (swipe_to_find_layer(layer_num)) {
        return true;
    }

    LOG_ERROR("Failed to select layer: %d", layer_num);
    return false;
}
...
bool OrochiLayerSelector::find_layer_directly(int layer_num)
{
    ...
    LOG_DEBUG("Waiting for target: %s, timeout: %dms", layer_target.c_str(), DEFAULT_TIMEOUT_MS);

    while (true) {
        const auto rect = m_ctx->resolver()->find_template(layer_target);
        if (rect && !rect->empty()) {
            LOG_DEBUG("Target %s found at (%d, %d)", layer_target.c_str(), rect->x, rect->y);
            return true;
        }
        ...
        if (elapsed_ms >= DEFAULT_TIMEOUT_MS) {
            LOG_DEBUG("Layer %d not found directly", layer_num);
            return false;
        }
...
bool OrochiLayerSelector::swipe_to_find_layer(int target_layer)
{
    if (!m_ctx || !m_ctx->executor()) {
        LOG_ERROR("Executor not available for swipe");
        return false;
    }

    LOG_INFO("Swiping to find layer: %d", target_layer);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        if (find_layer_directly(target_layer)) {
            LOG_INFO("Found layer %d after %d swipes", target_layer, i);
            return click_layer(layer_target);
        }

        LOG_DEBUG("Performing swipe %d, direction: %s", i, swipe_up ? "up" : "down");
        if (!m_ctx->executor()->swipe(from, to)) {
            LOG_WARN("Swipe failed on attempt %d", i + 1);
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
    }

    LOG_WARN("Layer %d not found after swiping", target_layer);
    return false;
}
...
bool OrochiLayerSelector::click_layer(const std::string& layer_target)
{
    ...
    for (int i = 0; i < max_retry; ++i) {
        LOG_DEBUG("Click attempt %d for target: %s", i + 1, layer_target.c_str());

        const auto rect = m_ctx->resolver()->find_template(layer_target);
        if (!rect || rect->empty()) {
            LOG_DEBUG("Template not found: %s", layer_target.c_str());
        ...
        LOG_DEBUG("Target %s found at (%d, %d), size: %dx%d", layer_target.c_str(), x, y, rect->width, rect->height);

        if (m_ctx->executor()->click(Point { x, y })) {
            LOG_INFO("Clicked %s at (%d, %d)", layer_target.c_str(), x, y);
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
            return true;
        }

        LOG_WARN("Click failed for %s, attempt: %d", layer_target.c_str(), i + 1);
        m_ctx->executor()->screencap();
        std::this_thread::sleep_for(std::chrono::milliseconds(300));
    }

    LOG_ERROR("Failed to click %s after %d attempts", layer_target.c_str(), max_retry);
    return false;
}
