...
YYSOrochiTask::YYSOrochiTask()
    ...
    , m_battle_count(0)
{
    YYS_LOG_INFO("YYSOrochiTask constructed, target layer: %d", static_cast<int>(m_target_layer));
}

bool YYSOrochiTask::_run()
{
    YYS_LOG_INFO("========== YYSOrochiTask started ==========");
    YYS_LOG_INFO("Target layer: %d, Sweep enabled: %s, Sweep count: %d",
    ...
    if (!check_medicine()) {
        YYS_LOG_ERROR("Medicine check failed, not enough stamina");
        return false;
    }
    ...
    if (!m_navigator->enter()) {
        YYS_LOG_ERROR("Failed to enter orochi interface");
        return false;
    }

    if (!m_layer_selector->select(static_cast<int>(m_target_layer))) {
        YYS_LOG_ERROR("Failed to select layer: %d", static_cast<int>(m_target_layer));
        return false;
    }
    ...
    for (int i = 0; i < max_battles; ++i) {
        YYS_LOG_INFO("Starting battle #%d", i + 1);

        if (!m_battle_runner->start()) {
            YYS_LOG_ERROR("Failed to start battle #%d", i + 1);
            return false;
        }

        if (!m_battle_runner->wait_for_end()) {
            YYS_LOG_ERROR("Failed to wait battle #%d end", i + 1);
            return false;
        }

        ++m_battle_count;
        YYS_LOG_INFO("Battle #%d completed", i + 1);
        m_reward_handler->handle();

        if (i < max_battles - 1) {
            if (!check_medicine()) {
                YYS_LOG_WARN("Not enough stamina for next battle, stopping");
                break;
            }
    ...
    YYS_LOG_INFO("========== YYSOrochiTask completed, total battles: %d ==========", m_battle_count);
    return true;
}

bool YYSOrochiTask::on_executed()
{
    YYS_LOG_INFO("YYSOrochiTask on_executed called, battles completed: %d", m_battle_count);
    return true;
}

bool YYSOrochiTask::check_medicine()
{
    YYS_LOG_DEBUG("Checking medicine/stamina...");
    YYS_LOG_DEBUG("Medicine check passed (placeholder)");
    return true;
}
